 ...
 */
export function speakPoint(point: { date: string; value: number }, index: number, total: number): void {
  if (typeof window === "undefined") return;
  ...
  const dateFormatted = new Date(point.date + "T00:00:00").toLocaleDateString("en-US", {
    month: "long",
    day: "numeric",
  });
