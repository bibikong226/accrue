/**
 * buildChartNarration -- produces a plain-English narration paragraph
 * for any price-history series. Always uses `value` (not `close`)
 * to match the DataPoint type from mockPortfolio.ts.
 *
 * § 6 of the Chart Rebuild Spec.
 */
...
export function buildChartNarration(input: NarrationInput): string {
  ...
  if (points.length < 2)
    return `Not enough data to summarize ${ticker} over this period.`;
  ...
  return (
    `This is ${ticker} over the last ${timeframe}. ` +
    `It started near $${first.value.toFixed(2)}, ${direction} by ${last.date}, ` +
    `with a peak of $${high.value.toFixed(2)} on ${high.date} and ` +
    `a low of $${low.value.toFixed(2)} on ${low.date}. ` +
    `Overall direction: ${deltaPct > 1 ? "up" : deltaPct < -1 ? "down" : "sideways"}. ` +
